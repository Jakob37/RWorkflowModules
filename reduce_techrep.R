...
sdf <- rdf %>% select(as.character(ddf[[argv$sample_col]])) %>% as.data.frame()
adf <- rdf %>% select(-one_of(as.character(ddf[[argv$sample_col]]))) %>% as.data.frame()

red_mats <- reduce_technical_replicates_for_matrices(designMat = ddf, dataMat = sdf, ddf[[argv$techrep_col]])
