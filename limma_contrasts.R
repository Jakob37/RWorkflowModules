...
stat_sdf <- sdf %>% dplyr::select(-one_of(argv$omit_samples))

message("Loaded raw data with dimensions: ", paste(dim(rdf), collapse=", "))

# Prepare R equation objects
factor_ddf <- apply(ddf, 2, as.factor)
model <- as.formula(argv$model)
model_design <- model.matrix(model, data=factor_ddf)
...
# Calculate Limma table (F-statistic)
